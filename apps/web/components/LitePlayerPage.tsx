...
.video-stable,.video-stable *{animation:none!important;transition:none!important;transform:none!important;opacity:1!important;filter:none!important}
.video-stable video{display:block!important;visibility:visible!important;backface-visibility:hidden}
...
(function(){
  ...
  function trans(s){if(isVideoSlide(s))return 'video-stable';var t=(s&&s.transition)||(s&&s.metadata&&s.metadata.slideTransition)||'fade';return 'slide-in-'+String(t).replace(/[^a-z-]/g,'');}
  ...
  function renderTemplate(items,total){
    ...
    for(var i=0;i<items.length;i++){var it=items[i],d=it.data||{},url=media(d.sourceUrl||''),content=String(d.content||d.label||''),left=(it.x/760*100),top=(it.y/430*100),w=(it.width/760*100),h=(it.height/430*100);
      ...
      var itemStart=it.kind==='video'?0:startOf(d),itemEnd=it.kind==='video'?total:endOf(d,total);
      html+='<div class="template-item '+(it.kind==='video'?'video-stable':'')+'" data-start="'+itemStart+'" data-end="'+itemEnd+'" data-enter="'+esc(it.kind==='video'?'none':d.transition||'none')+'" data-exit="'+esc(it.kind==='video'?'none':d.exitTransition||d.transition||'none')+'" style="'+pos+'z-index:'+(it.zIndex||1)+';'+itemCss(it,d)+'">';
  ...
  function render(s){
    ...
    else if(typ==='video'){var fmv=fit(s);html='<div class="media-stage fit-'+fmv+'"><video class="full-media fit-'+fmv+'" src="'+esc(u)+'" autoplay muted loop playsinline webkit-playsinline preload="auto"></video><div class="video-error">Nao foi possivel reproduzir este video nesta TV. Reenvie este video depois da atualizacao para converter no padrao LG.</div></div>';}
  ...
  function isVideoSlide(s){return !!(s&&(mediaType(s.type,s.body)==='video'||(s.type==='template'&&hasVideo(s.body||[]))));}
  function playVideos(){var videos=document.getElementsByTagName('video');for(var i=0;i<videos.length;i++){(function(v){try{v.muted=true;v.autoplay=true;v.playsInline=true;v.setAttribute('muted','');v.setAttribute('autoplay','');v.setAttribute('playsinline','');v.setAttribute('webkit-playsinline','');var e=document.querySelector&&document.querySelector('.video-error');function showError(){if(e)e.style.display='block';}function hideError(){if(e)e.style.display='none';}v.onerror=showError;v.onstalled=showError;v.onplaying=hideError;var tries=0;function go(){tries++;try{if(v.readyState===0&&v.load)v.load();var p=v.play&&v.play();if(p&&p.catch)p.catch(function(){if(tries<12)setTimeout(go,700);else showError();});if(v.paused&&tries<12)setTimeout(go,700);}catch(err){if(tries<12)setTimeout(go,700);else showError();}}v.onloadedmetadata=go;v.oncanplay=go;go();setTimeout(function(){try{if((v.paused||v.readyState<2)&&v.currentTime<0.2)showError();}catch(err){}},5000);}catch(e){}})(videos[i]);}}
